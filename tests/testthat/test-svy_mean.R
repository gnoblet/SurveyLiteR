# Test file for svy_mean function
# Tests compare SurveyLiteR svy_mean function with direct srvyr calls

# Helper function to create test data for svy_mean
create_mean_test_data <- function() {
  set.seed(123)
  data.frame(
...
create_mean_survey_design <- function(data) {
  srvyr::as_survey_design(data, weights = weights, strata = strata)
}

test_that("svy_mean produces correct estimates without grouping", {
  data <- create_mean_test_data()
  design <- create_mean_survey_design(data)

  # Test single variable
  result_svy <- svy_mean(design, vars = "numeric_var1", ak = FALSE)

  # Direct srvyr comparison
  direct_result <- design %>%
    srvyr::summarise(
      stat = srvyr::survey_mean(numeric_var1, na.rm = TRUE, vartype = "ci"),
      .groups = "drop"
    )
  ...
  expect_equal(result_svy$stat_type[1], "mean")
  expect_true(is.na(result_svy$var_value[1]))
})

test_that("svy_mean produces correct estimates with single grouping", {
  data <- create_mean_test_data()
  design <- create_mean_survey_design(data)

  # Test with grouping
  result_grouped <- svy_mean(
    design,
    vars = "numeric_var1",
    group = "group_var1",
    ak = FALSE
...
test_that("svy_mean produces correct estimates with multiple grouping", {
  data <- create_mean_test_data()
  design <- create_mean_survey_design(data)

  # Test with multiple grouping variables
  result_multi_group <- svy_mean(
    design,
    vars = "numeric_var1",
...
test_that("svy_mean handles multiple variables correctly", {
  data <- create_mean_test_data()
  design <- create_mean_survey_design(data)

  # Test multiple variables
  result_multi <- svy_mean(
    design,
  ...
  expect_true(all(result_multi$stat_type == "mean"))

  # Check that each variable gets correct mean
  for (i in 1:3) {
...
test_that("svy_mean handles different variance types", {
  data <- create_mean_test_data()
  design <- create_mean_survey_design(data)
...
test_that("svy_mean handles NA values correctly", {
  # Create data with NAs
  data_with_na <- create_mean_test_data()
  data_with_na$numeric_var1[1:10] <- NA

  design <- create_mean_survey_design(data_with_na)

  # Test with na_rm = TRUE
  result_na_rm <- svy_mean(
    design,
    vars = "numeric_var1",
    na_rm = TRUE,
    ak = FALSE
  ...
  expect_equal(result_na_rm$na_count_tot[1], 10)
  expect_equal(result_na_keep$na_count_tot[1], 10)
...
test_that("svy_mean respects confidence levels", {
  data <- create_mean_test_data()
  design <- create_mean_survey_design(data)
...
test_that("svy_mean analysis key functionality works", {
  data <- create_mean_test_data()
  design <- create_mean_survey_design(data)

  # Test with analysis key
  result_ak <- svy_mean(design, vars = "numeric_var1", ak = TRUE)
  result_no_ak <- svy_mean(design, vars = "numeric_var1", ak = FALSE)

  expect_true("analysis_key" %in% colnames(result_ak))
  expect_false("analysis_key" %in% colnames(result_no_ak))
  expect_true(grepl("mean", result_ak$analysis_key[1]))
  expect_true(grepl("numeric_var1", result_ak$analysis_key[1]))

  # Test with grouping
  result_grouped_ak <- svy_mean(
    design,
    vars = "numeric_var1",
    group = "group_var1",
    ak = TRUE
  ...
  expect_true(all(grepl("mean", result_grouped_ak$analysis_key)))
...
test_that("svy_mean calculates unweighted statistics correctly", {
  data <- create_mean_test_data()
  design <- create_mean_survey_design(data)

  result <- svy_mean(design, vars = "numeric_var1", ak = FALSE)
  expected_unweighted <- mean(data$numeric_var1, na.rm = TRUE)

  expect_equal(result$stat_unw[1], expected_unweighted, tolerance = 1e-10)
})

test_that("svy_mean handles edge cases", {
  ...
  design <- create_mean_survey_design(minimal_data)
  result <- svy_mean(design, vars = "x", ak = FALSE)

  expect_equal(nrow(result), 1)
  expect_true(is.numeric(result$stat[1]))
  expect_equal(result$stat[1], 2) # Mean of 1, 2, 3
  ...
  design_constant <- create_mean_survey_design(constant_data)
  result_constant <- svy_mean(design_constant, vars = "x", ak = FALSE)

  expect_equal(result_constant$stat[1], 5)
  expect_equal(result_constant$stat_low[1], result_constant$stat_upp[1]) # CI should collapse to point
})

test_that("svy_mean handles all NA variables correctly", {
  # Create data with all NAs
  data_all_na <- create_mean_test_data()
  data_all_na$numeric_var1 <- rep(NA_real_, nrow(data_all_na))

  design <- create_mean_survey_design(data_all_na)

  # Should return empty data frame with warning
  expect_warning(
    result_all_na <- svy_mean(design, vars = "numeric_var1", ak = FALSE),
    "only contains missing values"
  )
  expect_equal(nrow(result_all_na), 0)
})

test_that("svy_mean parameter validation works", {
  data <- create_mean_test_data()
  design <- create_mean_survey_design(data)

  # Test invalid vartype (error comes from srvyr, not our validation)
  expect_error(
    svy_mean(design, vars = "numeric_var1", vartype = "invalid"),
    "'arg' should be one of"
  )

  # Test invalid confidence level
  expect_error(
    svy_mean(design, vars = "numeric_var1", level = 1.5),
    "Element 1 is not <= 1"
  )

  # Test missing variables
  expect_error(
    svy_mean(design, vars = "nonexistent_var"),
    "Missing variables"
  )
})
