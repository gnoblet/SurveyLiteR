...
#'   Variables to calculate the proportoins from.
...
svy_proportion <- function(
  ...
  analysis <- svy_make(
    ...
    # wrapper function that ignores the variable argument and only passes the other parameters (`vartype`, `level`, etc.)
    survey_fn = function(var, ...) srvyr::survey_prop(proportion = TRUE, ...),
    unweighted_fn = function(x) 1, # Placeholder, will be recalculated
    group_by_var = TRUE,
    var_column_name = "var",
    add_var_value = FALSE, # Already handled by group_by_var
    ...
  )

  # Convert var_value to character for proportions
  if (nrow(analysis) > 0) {
    analysis[, var_value := as.character(var_value)]
  }
